...
for j = 2:M-1         % COLUMNS
    for i = 2:N-1     % ROWS
        %% Top
        F1C(1,j) = -((u(1,j+1)*u(1,j+1) - u(N,j-1)*u(N,j-1))/(2*mesh.dx)) - ((2*u(2,j)*v(2,j))/(mesh.dy));
...
% Top Left Corner
F1C(1,1) = -((u(1,2)*u(1,2) - u(1,M-1)*u(1,M-1))/(2*mesh.dx)) - ((2*u(2,1)*v(2,1))/(mesh.dy));
...
% Top Right Corner
F1C(1,M) = -((u(1,2)*u(N,2) - u(1,M-1)*u(1,M-1))/(2*mesh.dx)) - ((2*u(2,M))/(2*mesh.dy));
end       