...
for i = 2:N-1
    for j = 2:M-1
        %% Top
        F2C(1,j) = -((u(2,j)*v(2,j) - (u(N-1,j)*v(N-1,j)))/(2*mesh.dx)) - ((v(1,j+1)*v(1,j+1) - v(1,j-1)*v(1,j-1))/(2*mesh.dy));
        ...
        % Bottom Left Corner
        F2C(N,1) = ((v(N,2)*v(N,2)) - (-v(N,2)*-v(N,2)))/(2*mesh.dy);
        
        %% Main Domain
        F2C(i,j) = ((u(i+1,j)*v(i+1,j) - u(i-1,j)*v(i-1,j))/(2*mesh.dx)) - ((v(i,j+1)*v(i,j+1) - v(i,j-1)*v(i,j-1))/(2*mesh.dy));
        
        %% Bottom
        F2C(N,j) = -((u(2,j)*v(2,j)) - (u(N-1,j)*v(N-1,j)))/(2*mesh.dx) - ((v(N,j+1)*v(N,j+1) - v(N,j-1)*v(N,j-1))/(2*mesh.dy));
        
        % Bottom Right Corner
        F2C(N,M) = (-v(N,M-1)*v(N,M-1))/(2*mesh.dy);
